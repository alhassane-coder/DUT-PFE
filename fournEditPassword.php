	    ...
	    extract($_POST);
		 ...
		 if(mb_strlen($new_password) < 6){
		            ...
		            if(count($errors) == 0){
			            ...
			            $q = $db->prepare('SELECT password as hashed_password FROM fournisseurs WHERE id=? ');
                        
                        $q->execute([get_session('fourn_id')]);
			            ...
			            if($passwordHasBeenFound && password_verify($current_password,$data->hashed_password )){
			            	...
			            	$q = $db->prepare('UPDATE fournisseurs SET password = :new_password WHERE id=:id');
			            	...
			            	$success = $q->execute([
                             ...
                             if($success){
								...
								$event='le fournisseur <span style="color:blue">'.$fourniss_infos->name.'</span> a modifié son mot de passe';
								$q->execute(array(
								'event'=>$event,
								'date'=>$date
								));
...
include "views/fournEditPassword.view.php"; 
