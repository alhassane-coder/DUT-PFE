    ...
    <div class="main-content">
        ...
        <main>
            ...
            <?php foreach($informaticiens as $informaticien) : ?>
                <tr>
                  <td><?= $informaticien->id ?></td>
                  <td><?= $informaticien->name ?></td>
                  <td><?= $informaticien->firstName ?></td>
                  <td><?= $informaticien->email ?></td>
                  <td><?= $informaticien->tel ?></td>
                  <td>
                      <a onclick="return confirm('Voulez vous vraiment supprimer cet informaticien ?');" class="submit-btn1" href="delInformaticien.php?id=<?=$informaticien->id ?>"><i class="fas fa-trash-alt"></i> Supprimer</a><br><br>
                     <?php if($informaticien->active == 1) : ?>
                      <a onclick="return confirm('Voulez vous vraiment bloquer cet informaticien ?');" class="submit-btn2" href="blockInformaticien.php?id=<?=$informaticien->id ?>"><i class="fas fa-stop-circle"></i> Bloquer</a>
                      <?php else: ?>
                      <a onclick="return confirm('Voulez vous vraiment débloquer cet informaticien ?');" class="submit-btn3" href="deblockInformaticien.php?id=<?=$informaticien->id ?>"><i class="fas fa-check-circle"></i> Debloquer</a>
                      <?php endif; ?>
                </td>
